# Ranger Wildtrak from Korea — пошук і пригін Ford Ranger

## Що це за проєкт
**Особистий** (не робочий) проєкт Володимира: підбір вживаного **Ford Ranger Wildtrak**
...
## Критерії (чинні з 2026-09-24)
- **Модель:** Ford Ranger, комплектація **Wildtrak** (в Encar `BadgeDetail` = `와일드트랙`;
  Raptor `랩터` не беремо). Двигун у всіх один — 2.0 дизель (`Badge` = `2.0`).
- **Рік виготовлення:** 2022 і новіші. Беремо `Year // 100` з пошуку Encar (виготовлення),
  а **не** `FormYear` (연식, модельний). Через це поза списком лишаються лоти
  **модельного 2022, але випуску 2021** (напр. `42252727` 2690만 і `42707629` 2850만 —
  обидва з `Year` 2021-06/07, `FormYear` 2022). Якщо користувач скаже, що модельний рік
  теж годиться — змінити `Year.range` у `tools/encar.py::_query` на `FormYear`.
...
Показуємо: рік, покоління, пробіг, ціна в Кореї (만원 + `≈ $` за курсом 1372 ₩/$),
...
- **Пошук:** `https://api.encar.com/search/car/list/general?count=true&q=<QUERY>&sr=|ModifiedDate|<offset>|20`.
  Робочий запит для Ranger Wildtrak:
  ```
  (And.Hidden.N._.(C.CarType.A._.(C.Manufacturer.포드._.ModelGroup.레인저.))_.Year.range(202200..)._.SellType.일반._.BadgeDetail.와일드트랙.)
  ```
  ...
  **Дублів дуже багато:** з 26 лотів пошуку 8 — той самий VIN під другим `listingId`,
  ще 4 — уже за контрактом. Дедуплікація обов'язкова.
- **Деталь:** `https://api.encar.com/v1/readside/vehicle/{listingId}` → `vehicleId`, `vin`,
  ...
  **⚠️ `vehicleId` ≠ `listingId`** — брати з деталі (`vehicleId` або `manage.dummyVehicleId`).
  VIN Ford: `6FPPXXMJ2N…` (3 пок.) / `6FPP2CMJ…` (4 пок.), 10-й знак — код року
...
1. **Головна** `index.html` — одна таблиця, 8 колонок: `№ · Авто · Пробіг · Ціна в Кореї ·
   ДТП · Ремонт · Змін вл. · Нотатка`. Сортування кліком по заголовку, тай-брейк за ціною;
   `№` = місце за ціною. Три перші колонки липкі по горизонталі, шапка — по вертикалі.
   Фільтри: покоління / рік (з даних) / різних ДТП не більше / ремонт. Фільтр ДТП ховає й
...
2. **Сторінка авто** `car.html?id=<listingId>` — галерея, «Що це за авто» (VIN, покоління,
...
tools/encar.py           клієнт Encar: search(year_from), detail, record, inspection, photos
...
```
Поля індексу: `rank, listingId, model ("Ranger Wildtrak"), gen (3|4), year, mileageKm,
